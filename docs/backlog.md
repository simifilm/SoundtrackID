...
| 2.5 | Test **Apple ShazamKit** (Shazam via `shazamio` weak/blank) | 🟡 | Native Swift helper (`shazamkit/`) built; `ShazamDetectionClient` calls it (shazamio removed). Works only once the App ID `ch.uzh.soundtrackid` has the ShazamKit capability + a Developer ID provisioning profile (portal step by UZH). Dev builds return 202 -> graceful no-match. |
